...
$cekPassword = mysqli_query($koneksi, "SELECT * FROM tb_mahasiswa WHERE password_mhs = '$password_mhs' ");
if($cekPassword -> num_rows > 0) {
  ...
  if(!empty($password1 && $password2)){
    if($password1 == $password2){
      ...
      echo "<script> 
          alert ('Password Gagal di Update'); window.location = 'profile.php' </script> ";
    }
  }
}
